...
| `tokentrim/tracing/` | Canonical trace records, trace stores, and pipeline tracing interfaces |
| `tokentrim/working_state.py` | Shared working-state extraction, rendering, and parsing |
| `tokentrim/salience.py` | Shared ranking heuristics used by ranking helpers |
...
- keep the compaction pipeline simple and model-driven
- allow caller-provided prompt instructions without changing deterministic preprocessing

Current output shape:

- a working-state system block
- a compacted-history system block generated by the compaction model
- preserved recent live messages

The summary prompt defaults to a structured engineering handoff. Callers may
override the primary prompt instructions, but output still comes from the same
deterministic preprocessing plus single compaction-model call.
